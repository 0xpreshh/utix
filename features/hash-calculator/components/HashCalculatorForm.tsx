...
const defaults:RawInput={"mode": "SHA-256", "encoding": "UTF-8", "value": "", "network": "Testnet", "passphrase": ""};
const secretFields:string[]=[];
export function HashCalculatorForm({onSubmit,onEdit,pending}:{onSubmit:(raw:RawInput)=>void;onEdit:()=>void;pending:boolean}){
 const [values,setValues]=useState<RawInput>(defaults);const [rejected,setRejected]=useState(false);
 function change(key:string,value:string){
  onEdit();
  if((secretFields.includes(key) && /^S/i.test(value.trim())) || /^S[A-Z2-7]{55}$/.test(value.trim())){setValues(v=>({...v,[key]:""}));setRejected(true);return;}
