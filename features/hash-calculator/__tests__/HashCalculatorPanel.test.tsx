...
it("renders a useful result and clears it on edits and reset",async()=>{const {user}=renderFeature(<HashCalculatorPanel/>);expect(screen.getByText(copy.emptyTitle)).toBeInTheDocument();await fill(user);await user.click(screen.getByRole("button",{name:copy.submit}));expect(await screen.findByText(copy.resultTitle)).toBeInTheDocument();await user.click(screen.getByRole("button",{name:copy.reset}));expect(screen.queryByText(copy.resultTitle)).not.toBeInTheDocument();expect(screen.getByText(copy.emptyTitle)).toBeInTheDocument();});
it("shows input errors once",async()=>{const {user}=renderFeature(<HashCalculatorPanel/>);await user.click(screen.getByRole("button",{name:copy.submit}));expect(await screen.findByRole("alert")).toBeInTheDocument();});
