...
import {unexpectedFailure} from "../lib/trustlineLimitPlanner.errors";
import type {RawInput,Report,ErrorCode} from "../types";
export type State = {status:"idle"}|{status:"loading"}|{status:"success";result:Report}|{status:"error";code:ErrorCode};
export function useTrustlineLimitPlanner() {
 ...
 const submit=useCallback(async(raw:RawInput)=>{
  ...
  try{const result=await analyze(parsed.value);if(id!==generation.current)return;setState(result.ok?{status:"success",result:result.value}:{status:"error",code:result.code});}
