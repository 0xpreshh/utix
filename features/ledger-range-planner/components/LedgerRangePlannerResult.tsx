...
export function LedgerRangePlannerResult({result}:{result:Report}){
 ...
 <CopyableValue label={copy.report} value={formatReport(result)}/><a download="ledger-range-planner.json" href={"data:application/json;charset=utf-8,"+encodeURIComponent(formatReport(result))}>{copy.download}</a>
 </Card>;
}
