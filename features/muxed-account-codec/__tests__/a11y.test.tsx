...
it("passes axe initially",async()=>{const {container}=renderFeature(<MuxedAccountCodecPanel/>);await expectNoAxeViolations(container);});
it("passes axe with a result",async()=>{const {container,user}=renderFeature(<MuxedAccountCodecPanel/>);await fill(user);await user.click(screen.getByRole("button",{name:copy.submit}));await screen.findByText(copy.resultTitle);await expectNoAxeViolations(container);});
