...
const defaults:RawInput={"snapshot": "", "limit": ""};
const secretFields:string[]=[];
export function TrustlineLimitPlannerForm({onSubmit,onEdit,pending}:{onSubmit:(raw:RawInput)=>void;onEdit:()=>void;pending:boolean}){
 const [values,setValues]=useState<RawInput>(defaults);const [rejected,setRejected]=useState(false);
 function change(key:string,value:string){
  onEdit();
  if((secretFields.includes(key) && /^S/i.test(value.trim())) || (key === "snapshot" && /"asset_issuer"\s*:\s*"S/i.test(value))){setValues(v=>({...v,[key]:""}));setRejected(true);return;}
